...
interface ICircuitBreaker {
    ...
    // todo: use trigger instead of breach, or switch trigger to breach everywhere
    ...
    /// @dev MUST be emitted in `registerToken` when a token is registered
    /// @param token MUST be the address of the asset for which to set rate limit parameters. 
    /// For any EIP-20 token, MUST be an EIP-20 token contract.
    /// For native ETH, MUST be address 0x0000000000000000000000000000000000000001 equivalent to address(1).
    ...
    event TokenRegistered(address indexed token, uint256 metricThreshold, uint256 minAmountToLimit);

    /// @dev MUST be emitted in `onTokenInflow` and `onTokenInflowNative` by during asset inflow into a protected contract
    ...
    /// @dev MUST be emitted in `onTokenOutflow` and `onTokenOutflowNative` when a rate limit is triggered
    /// @param token MUST be the address of the asset triggering the rate limit. 
    /// For any EIP-20 token, MUST be an EIP-20 token contract.
    /// For native ETH, MUST be address 0x0000000000000000000000000000000000000001 equivalent to address(1).
    ...
    /// @dev MUST be emitted in `onTokenOutflow` and `onTokenOutflowNative` when an asset is successfully withdrawn
    /// @param token MUST be the address of the asset withdrawn. 
    /// For any EIP-20 token, MUST be an EIP-20 token contract.
    /// For native ETH, MUST be address 0x0000000000000000000000000000000000000001 equivalent to address(1).
    ...
    /// @param token MUST be the address of the asset claimed. 
    /// For any EIP-20 token, MUST be an EIP-20 token contract.
    /// For native ETH, MUST be address 0x0000000000000000000000000000000000000001 equivalent to address(1).
    ...
    /// @dev Each asset that will be rate limited MUST be registered using this function, including native ETH. 
    ...
    /// @param _token The address of the asset for which to set rate limit parameters. 
    /// To set the rate limit parameters for any EIP-20 token, MUST be an EIP-20 token contract.
    /// To set rate limit parameters for native ETH, MUST be address 0x0000000000000000000000000000000000000001 equivalent to address(1).
    ...
    /// MUST revert if asset is not previously registered with the registerToken method.
    /// MUST revert if the caller is not the current admin.
    /// @param _token The address of the asset contract for which to set rate limit parameters. 
    /// To update the rate limit parameters for any EIP-20 token, MUST be an EIP-20 token contract.
    /// To update the rate limit parameters for native ETH, MUST be address 0x0000000000000000000000000000000000000001 equivalent to address(1).
    /// @param _metricThreshold The threshold metric which defines when a rate limit is triggered. 
    ...
    /// MUST emit event TokenInflow on successful call.
    ...
    /// If a rate limit is triggered and the circuit breaker is not in grace period, this method MUST revert and MUST emit event TokenRateLimitBreached.
    ...
    /// MUST revert if caller is not a protected contract.
    /// MUST emit event TokenWithdraw on successful call.
    /// @param _token MUST be an EIP-20 token contract
    ...
    /// @notice Record native ETH inflow into a protected contract
    /// @dev This method MUST be called from all protected contract methods where native ETH is transferred in from a user.
    /// MUST revert if caller is not a protected contract.
    /// MUST emit event TokenInflow on successful call.
    /// @param _amount MUST equal the amount of native ETH transferred into the protected contract
    function onTokenInflowNative(uint256 _amount) external;

    /// @notice Record native ETH outflow from a protected contract and transfer ETH to recipient if rate limit is not triggered
    /// @dev This method MUST be called from all protected contract methods where native ETH is transferred out to a user.
    /// When calling this method, the protected contract MUST send the native ETH to the circuit breaker contract in the same call.
    /// For an example, see ProtectedContract.sol in the reference implementation.
    /// If native ETH is not registered, this method MUST NOT revert and MUST transfer the native ETH to the recipient.
    /// If a rate limit is not triggered or the circuit breaker is in grace period, this method MUST NOT revert and MUST transfer the ETH to the recipient.
    /// If a rate limit is triggered and the circuit breaker is not in grace period, this method MUST revert and MUST emit event TokenRateLimitBreached.
    /// If a rate limit is triggered and the circuit breaker is not in grace period, this method MUST record the locked funds in the internal accounting of the circuit breaker implementation.
    /// MUST revert if caller is not a protected contract.
    /// MUST emit event TokenWithdraw on successful call.
    /// @param _recipient MUST be the address of the recipient of the transferred ETH from the protected contract
    /// @param _revertOnRateLimit MUST be TRUE to revert if a rate limit is triggered or FALSE to return without reverting if a rate limit is triggered (delayed settlement)
    ...
    /// @dev When a token is transferred out during a rate limit period, the settlement may be delayed and the token custodied in the circuit breaker.
    ...
    /// MUST revert if the recipient does not have locked funds for a given token or native ETH.
    /// MUST revert if circuit breaker is rate limited and not in grace period.
    /// MUST emit event LockedFundsClaimed on successful call.
    /// MUST transfer tokens or native ETH to the recipient on successful call.
    /// MUST update internal accounting of circuit breaker implementation to reflect withdrawn balance on successful call.
    /// @param _token To claim locked EIP-20 tokens, this MUST be an EIP-20 token contract.
    ...
    /// The admin has authority to: withdraw locked funds, set grace periods, register token parameters, update token parameters, 
    /// set new admin, override rate limit, add protected contracts, remove protected contracts.
    /// MUST revert if the caller is not the current admin.
    /// MUST revert if the new admin is _address(0).
    /// MUST emit event AdminSet on successful call.
    ...
    /// Protected contracts have the authority to trigger rate limits and withdraw tokens. 
    /// MUST revert if caller is not the current admin.
    ...
    // TODO - other requirements? method not implemented yet.
    ...
    /// @param token To view the balance of locked EIP-20 tokens, this MUST be an EIP-20 token contract.
    /// To claim native ETH, this MUST be address 0x0000000000000000000000000000000000000001 equivalent to address(1).
    ...
    /// @notice Check if a rate limit is currently breeched for a given asset
    /// @param _token To check if breeched for EIP-20 tokens, this MUST be an EIP-20 token contract.
    /// To check if breeched for native ETH, this MUST be address 0x0000000000000000000000000000000000000001 equivalent to address(1).
    /// @return isRateLimitBreeched MUST return TRUE if a rate limit is currently breeched for given asset, FALSE if not
